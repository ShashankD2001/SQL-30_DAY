...
with cte as
( select user_id,spend, row_number() over (partition by user_id order by user_id) as rank_

from transactions
group by user_id , spend)
...
select top 5* from product_revenue
...
group by product_name,year,revenue) 
select  --top 5
product_name,prev_year_revenue ,revenue , (prev_year_revenue - revenue ) as difference_ , 
